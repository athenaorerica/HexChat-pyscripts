...
if sys.platform.startswith("linux"):
    jsonpath = "~/.config/Google Play Music Desktop Player/json_store/playback.json"
elif sys.platform.startswith("darwin"):
    jsonpath = "~/Library/Application Support/Google Play Music Desktop Player/json_store/playback.json"
elif sys.platform.startswith("win"):
    jsonpath = "{0}\\Google Play Music Desktop Player\\json_store\\playback.json".format(
        os.getenv('appdata'))


def prettyprint(status, message):
    hexchat.emit_print('Notice', __module_name__ +
                       ' [{0}]'.format(status), message)
...
def convertMillis(millis):
    ...
    timestr = ""
    if hours != 0:
        timestr = timestr + "{}:".format(str(hours))
