...
if hexchat.get_pluginpref('mpdnp_server') == None:
    formattedprint(
        'Running for the first time. Setting server to \'localhost\'. to change it, /mpdserver <new host>')
    hexchat.set_pluginpref('mpdnp_server', 'localhost')

if hexchat.get_pluginpref('mpdnp_port') == None:
    formattedprint(
        'Running for the first time. Setting port to 6600. to change it, /mpdport <new port>')
    hexchat.set_pluginpref('mpdnp_port', '6600')

if hexchat.get_pluginpref('mpdnp_pass') == None:
    formattedprint(
        'Running for the first time. Setting password protection to disabled. to enable it, /mpdpass <password>')
